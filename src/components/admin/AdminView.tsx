  ...
  if (mode === "add") {
    return (
      <div className="space-y-4">
        <SectionTitle icon={<Plus className="w-4 h-4" />}>Add New Project</SectionTitle>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {projects.map((p) => {
                const statusVariant =
                  p.status === "active" ? "green" : p.status === "ended" ? "red" : "amber";
                return (
                  <tr key={p.id} className="border-t border-border hover:bg-raised/50">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="w-7 h-7 rounded bg-brand/15 text-brand flex items-center justify-center text-xs font-bold">
                          {p.name[0]}
                        </div>
